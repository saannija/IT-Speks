...
<?php
    ...
    if(isset($_POST['reset'])){
        ...
        if($role == "admin"){
            ...
            $query = "SELECT Lietotajs_ID, Epasts FROM it_speks_lietotaji WHERE Lietotajvards = '{$username}'";
        }
        ...
        if (mysqli_num_rows($result) > 0) {
            while($user = mysqli_fetch_array($result)){
                ($role == "admin" ? $userId = $user['Darbinieks_ID'] : $userId = $user['Lietotajs_ID']);
                $email = $user['Epasts'];
            }    
            ...
            $resetLink = "http://kristovskis.lv/3pt1/romazanova/IT-Speks/assets/password_update.php?token=$token&role=$role";
